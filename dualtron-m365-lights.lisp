...
; Modified for Dualtron Thunder 3 brake light flickering by AI Assistant
; UART Wiring: red=5V black=GND yellow=COM-TX (UART-HDX) green=COM-RX (button)+3.3V with 1K Resistor
; Fan port used for brake light control
...
; Brake light flickering parameters
(def brake-flicker-enabled 1)
(def brake-flicker-freq 5) ; Hz frequency for brake light flickering
(def brake-threshold 0.2) ; Brake input threshold to trigger flickering
...
; Brake light flickering variables
(def brake-flicker-state 0)
(def brake-flicker-last-time 0)
(def brake-active 0)
(def brake-steady-active 0) ; For solid display brake light
(def current-brake-input 0)
...
(defun adc-input(buffer) ; Frame 0x65
    {
        (let ((current-speed (* (get-speed) 3.6))
            ...
            (brake (/(bufget-u8 uart-buf 5) 77.2)))
            {
                ...
                ; Store brake input for brake light logic
                (set 'current-brake-input brake)
...
; Initialize fan port for brake light control
(defun init-brake-light-port()
    {
        ; Configure fan port for PWM output
        (set-param 'app-general-aux-out-ch 0) ; Use AUX1 port
        (set-param 'app-general-aux-out-function 0) ; PWM output
        (set-param 'app-general-aux-out-invert 0) ; Normal polarity
    }
)

; Enhanced brake light control with fan port output
(defun control-brake-lights(brake-state)
    {
        (if (= fan-port-brake-enabled 1)
            {
                (if (= brake-state 1)
                    {
                        ; Turn on brake light via fan port
                        (set-aux 1 fan-port-duty-cycle)
                    }
                    {
...
(defun handle-brake-lights()
    {
        (var current-time (systime))
        (var brake-pressed (> current-brake-input brake-threshold))
        
        ; Set steady brake state for display (solid brake light)
        (set 'brake-steady-active brake-pressed)
        
        (if (and (= brake-flicker-enabled 1) brake-pressed)
            {
                ; Calculate flicker interval in milliseconds
                (var flicker-interval (/ 1000 (* brake-flicker-freq 2)))
                
                ; Check if it's time to toggle
                (if (> (- current-time brake-flicker-last-time) flicker-interval)
                    {
                        (set 'brake-flicker-state (bitwise-xor brake-flicker-state 1))
                        (set 'brake-flicker-last-time current-time)
                    }
                )
                
                (set 'brake-active brake-flicker-state)
                ; Control physical brake lights via fan port (flickering)
                (control-brake-lights brake-flicker-state)
            }
            {
                ; No brake or flickering disabled - turn off brake lights
                (set 'brake-active 0)
                (set 'brake-flicker-state 0)
                (control-brake-lights 0)
            }
        )
    }
)
...
(defun update-dash(buffer) ; Frame 0x64
    {
        ...
        ; light field - Modified to include solid brake light (no flickering on display)
        (if (= off 0)
            {
                (var light-state light)
                ; Add solid brake light to the light state for display
                (if (= brake-steady-active 1)
                    (set 'light-state (bitwise-or light-state 2)) ; Set brake light bit
                )
                (bufset-u8 tx-frame 8 light-state)
            }
...
; Alternative: Direct fan port control without flickering
(defun simple-brake-light-control()
    {
        (var brake-pressed (> current-brake-input brake-threshold))
        ...
        (if brake-pressed
            {
                ; Solid brake light when braking (fan port)
                (set-aux 1 fan-port-duty-cycle)
                (set 'brake-active 1)
            }
            {
...
; Initialize brake light port
(init-brake-light-port)
...
; BRAKE LIGHT CONFIGURATION NOTES:
; - M365 DISPLAY: Shows solid brake light indicator (no flickering)
; - FAN PORT: Physical brake lights flicker red to orange when braking
; - handle-brake-lights() provides flickering physical brake lights via fan port (default)
; - simple-brake-light-control() provides solid physical brake lights via fan port
; - To use solid brake lights instead of flickering, replace the call to
;   handle-brake-lights() in handle-features() with simple-brake-light-control()
; - Adjust fan-port-duty-cycle (0.0-1.0) for LED brightness
; - Connect brake light circuit to VESC fan port output
; - Set fan-port-brake-enabled to 0 to disable physical brake light control 
